...
if __name__ == "__main__":
    ...
    # Convert to matrix
    matrix = transformer.to_matrix()
    if matrix is not None:
        print(f"Matrix shape: {matrix.shape}")
        print(f"Matrix dimensions: {len(matrix.shape)}")
        print(f"Matrix data type: {matrix.dtype}")
        
        # More detailed information about dimensions
        if len(matrix.shape) == 3:
            height, width, channels = matrix.shape
            print(f"Image is {height} pixels high, {width} pixels wide, with {channels} color channels")
            
            # Print first row completely
            print("\nFirst row of pixels:")
            print(matrix[0, :])
            
            # Print 5x5 corner in a more structured way
            print("\n5x5 corner of the image (showing each pixel's RGB values):")
            for y in range(min(5, height)):
                row_pixels = []
                for x in range(min(5, width)):
                    row_pixels.append(matrix[y, x])
                print(f"Row {y}: {row_pixels}")
        
        print(f"\nOriginal matrix sample output:")
        print(f"Matrix sample values:\n{matrix[0:5, 0:5]}")
    
    # Example transformations
    if image:
        # Make a copy to demonstrate transformations
        transformed = ImageTransformer("sample_image.jpg")
        transformed.load_image()
        
        # Resize to 50% of original size
        width, height = image.size
        transformed.resize(width // 2, height // 2)
        
        # Apply a series of transformations using method chaining
        transformed.to_grayscale().adjust_contrast(1.5).blur_box(3)
        
        # Save the transformed image
        transformed.save_image("transformed_image.jpg")
